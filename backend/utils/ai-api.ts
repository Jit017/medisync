...
// Define the response structure to match our frontend expectations
...
// Function to generate AI response using local knowledge base
export async function generateAIResponse(userInput: string, userContext: UserContext): Promise<AIResponse> {
  try {
    // Check if the message is healthcare-related
    if (!isHealthcareRelated(userInput)) {
      return {
    ...
    for (const [symptom, symptomData] of Object.entries(healthcareKnowledge.symptoms)) {
      if (lowerText.includes(symptom.toLowerCase())) {
        const severity = determineSeverity(symptom, userInput);
